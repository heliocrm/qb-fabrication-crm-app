...
export function mapAccountRow(row: AccountRow): Account {
  return {
    ...
    state: row.state ?? "",
    status: row.status,
    qbCustomerUrl: row.qb_customer_url ?? null,
    qbCustomerId: row.qb_customer_id ?? null,
    qbStatusNote: row.qb_status_note ?? null,
    organizationId: row.organization_id,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    totalJobs: 0,
...
export function mapMaterialPullRequestRow(
  ...
  return {
    ...
    notes: row.notes,
    priority: (row.priority as MaterialPullRequest["priority"]) ?? "soon",
    reasonCode: (row.reason_code as MaterialPullRequest["reasonCode"]) ?? "other",
    sourceJobNumber: row.source_job_number ?? null,
    ...
    pullNotes: row.pull_notes ?? null,
    pullChecklist: (row.pull_checklist as MaterialPullRequest["pullChecklist"]) ?? null,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  }
}
